...
vignette: >
  %\VignetteIndexEntry{zoolog: Zooarchaeological Analysis with Log-Ratios}
...
We are grateful to all of them for their contributions, comments, and help. In addition, users are encouraged to contribute to the thesaurus and other references so that *zoolog* can be expanded and adapted to any database and language.
...
The package *zoolog* includes several osteometrical references. Currently, the references include reference values for the main domesticates and their agriotypes (*Bos*, *Ovis*, *Capra*, *Sus*), red deer (*Cervus elaphus*), Persian fallow deer (*Dama mesopotamica*), mountain gazelle (*Gazella gazella*), donkey (*Equus asinus*), horse (*Equus caballus*), European rabbit (*Oryctolagus cuniculus*), grey wolf (*Canis lupus*), and hen (*Gallus gallus*). These are drawn from a variety of publications and resources (see [reference](../help/reference) for a detailed list). In addition, the user can consider other references, or the provided references can be extended and updated integrating newer research data. Submission of extended/improved references is encouraged. Please, contact the maintainer through the provided email address [josmpozo@gmail.com] to make the new reference fully accessible within the package.
...
From *zoolog* release 2.0.0, the thesaurus set is internally structured by language. Currently, the available languages are
``` {r, echo=FALSE, results='asis'}
library(zoolog)
res <- zoolog:::FormatListOfNames(AllAvailableLanguages(), c("*", "*"))
cat(paste0(res, "."))
```
This allows the user to set which languages are [active](../help/ActiveLanguages). This modular structure facilitates the contribution of new languages by the user (see [creating new thesaurus languages](thesaurusLanguages.html)). In addition, the capacity to activate only the required languages, might avoid potential ambiguities between conflicting terms in different languages.

The structure in languages also allows the user to [set a language as the nomenclature standard](../help/ActiveLanguages). With this, the [nomenclature standardization](../help/StandardizeNomenclature) can convert any dataset to the desired language, effectively acting as a translator for the managed terms.

The user can also use other thesaurus sets or modify the provided one. In this latter case, we encourage the user to contact the maintainer at the provided email address [josmpozo@gmail.com] so that the additions can be incorporated into the new versions of the package.

The zoolog thesaurus set includes four thesauri:

identifierThesaurus
  : For the column names that typically identify different concepts in
    zooarchaeological data. This includes the variables used in computing
    the log ratios (categories *Taxon*, *Element*, *Measure*,
    and *Standard*) and many others structuring the data (such as *Site*,
    *Stratigraphic Unit*, *Chronology*, and *Laterality*, among others). Each
    category provides a series of equivalent names.
    For instance, with all languages active, *Taxon* includes the options
    `r a <- zoologThesaurus$identifier$Taxon;
      zoolog:::FormatListOfNames(a[a != ""], c("*", "*"))`.
...
taxonThesaurus
  : For the names of the different taxa when recording animal
    ...
    `r zoolog:::FormatListOfNames(names(zoologThesaurus$taxon), c("*", "*"))`,
...
elementThesaurus
  : Names of anatomical elements when recording animal bones. It currently
    ...
    (`r a <- names(zoologThesaurus$element)[1:3]
      zoolog:::FormatListOfNames(a, c("*", "*"), conjunction = "")`, ...),
    each with different equivalent names. 
    This thesaurus is case, accent, punctuation, and also word-order insensitive.
...
measureThesaurus
  : Names of the measurements. While the English abbreviations from
    ...
    abbreviations in @von1976guide) to be included. This thesaurus is case and
    punctuation sensitive.

# Taxonomy

A [taxonomy](../help/zoologTaxonomy) for the most typical zooarchaeological taxa was introduced in the *zoolog* major release 1.0.0 and has been extended in release 2.0.0. The taxonomic hierarchy is structured up to the class level:
``` {r, echo=FALSE}
...
[**CondenseLogs**](../help/CondenseLogs)
  : It condenses the calculated log ratio values into a reduced number of 
    ...
    extracted per group. Currently, three methods are possible: "priority" 
    (default), "average", or "random". 
  : This operation is motivated by two circumstances. First, not all 
    ...
    direct comparison and statistical analysis. Second, several measurements can
    be strongly correlated (e.g. SD and Bd both represent bone width).
    ...
    [@myers2013research; @Zimmerman2002]. In addition, it
    could produce an over-representation of bone remains with multiple 
    measurements per axis. Condensing each group of measurements into a single
    feature (e.g. one measure per axis) alleviates both problems.
  : The default method ("priority"), selects the first available log ratio in 
    ...
    *Depth* considers in order of priority  *Dd*, *DD*, *BG*, and *Dp*.
    The order of priority has been set on the basis of frequency and 
    ...
    dependent measurements (see @davis1996measurements for a discussion). 
    But users can set 
    their own features with any group of measures and priorities. 
    The method "average" extracts the mean per group, ignoring the 
    non-available log ratios. The method "random" randomly selects one measure
    log-ratio from the available ones per specimen and group.
...
[**Nomenclature standardization**](../help/StandardizeNomenclature)
  : This includes two functions enabling the user to map data with heterogeneous
    ...
    * `StandardizeDataSet` standardizes column names and values of a data 
    ...
    Remember that, when using the `zoologThesaurus`, the behaviour is 
    dependent on the active languages and on the standard language.
...
[**ActiveLanguages and StandardLanguage**](../help/ActiveLanguages)
  : This includes functions to get the inventory of available languages
    (`AllAvailableLanguages`) for the thesaurus, to get and set the active
    languages (`GetActiveLanguages` and `SetActiveLanguages`), and to get and
    set the standard language (`GetStandardLanguage` and `SetStandardLanguage`).
...
[**Thesaurus management**](../help/ThesaurusManagement)
  : This includes functions to modify and check thesauri:
    ...
    * `NewThesaurus` generates an empty thesaurus.
    * `AddToThesaurus` adds new terms and categories to an existing thesaurus.
    * `RemoveRepeatedNames` cleans a thesaurus from any repeated terms on any
      category.
    * `ThesaurusAmbiguity` checks if there are terms included in more than one
      category in a thesaurus.
    * `RemoveTermFromThesaurus` remove terms from a thesaurus.
    * `ChangeStandardInThesaurus` change the terms defining the standard on a
      thesaurus.
    * `RemoveCategory` remove categories from a thesaurus.
...
If we standardize the studied data, we can see that `zoologThesaurus` will change "bota" to "Bos taurus", "hum" to "Humerus", and "Os" to "Element", for instance.
```{r}
dataStandardized <- StandardizeDataSet(dataWithSummary)
knitr::kable(head(dataStandardized)[, -c(6:20,32:64,72:86)])
```

In contrast, if we first set the standard language to "Italian", for instance, the changes will be from "bota" to "Vacca", "hum" to "Omero", and "Os" to "Elemento".
```{r}
SetStandardLanguage("Italian")
dataStandardized <- StandardizeDataSet(dataWithSummary)
knitr::kable(head(dataStandardized)[, -c(6:20,32:64,72:86)])
SetStandardLanguage("Base")
```
In the last line, we reset the standard language to the default "Base".
...
For example, "Log ratios calculation and analysis was done using the package zoolog  [@pozo2023zoolog] in R 4.0.3 [@RCoreTeam2020]."
